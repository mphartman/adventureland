...
public class DefaultInterpreterTest {
    ...
    public void nextCommandShouldReturnPlayerCommandGivenValidVerbAndNoun() {
        Vocabulary vocabulary = new Vocabulary(new HashSet<Verb>(Arrays.asList(new Verb("JUMP"))), new HashSet<Noun>(Arrays.asList(new Noun("UP"))));
        DefaultInterpreter interpreter = new DefaultInterpreter(new Scanner("JUMP UP"));
        interpreter.setVocabulary(vocabulary);
    ...
    public void nextCommandShouldReturnUnrecognizedGivenVerbOrNounNotInVocabulary() {
        Vocabulary vocabulary = new Vocabulary(new HashSet<Verb>(Arrays.asList(new Verb("JUMP"))), new HashSet<Noun>(Arrays.asList(new Noun("UP"))));
        DefaultInterpreter interpreter = new DefaultInterpreter(new Scanner("KILL TROLL"));
        interpreter.setVocabulary(vocabulary);
        PlayerCommand playerCommand = interpreter.nextCommand();
